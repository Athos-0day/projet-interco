...
# Images
docker build -t services_fai_dns ./images/dns
docker build -t services_fai_web ./images/web
docker build -t services_fai_db ./images/db
...
# sudo ip link add <Le routeur service FAI> type veth peer name router0 netns services${SERVICES_ID}_switch
