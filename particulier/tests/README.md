...
Voici un schéma du réseau de test :
![](./docs/ReseauParticulierTest.png "Schema Reseau Test")
...
test/start.sh
```
- Ouvrir Wireshark sur le routeur FAI : 
```bash
sudo ip netns exec fai_particulierA wireshark 2> /dev/null &
```
- Ping le routeur FAI depuis particulierA_t1 : 
```bash
docker exec -it particulierA_t1 bash -c "ping 10.0.10.1"
```
